...
export class DataIngestionTask extends Construct {
  ...
  constructor(scope: Construct, id: string, props: IntTradesProps) {
      ...
      containerOverrides: [
        {
          ...
          environment: [
            ...
            {
              name: "TICK_SIZE",
              value: sfn.JsonPath.stringAt("$.tick_size"),
            },
            {
              name: "PRICE",
              value: sfn.JsonPath.stringAt("$.price"),
            },
          ],
        },
      ],
    ...
    const processTasks = new sfn.Map(this, "ProcessTasks", {
      ...
      outputPath: "$", // need to use $ to avoid Map state overrides the input state of generateTask
    });
    ...
    const concatParquetsTask = new tasks.LambdaInvoke(
      ...
      "ConcatParquetsTask",
      {
        ...
        inputPath: "$.GenerateTasksResult.Payload",
        payload: sfn.TaskInput.fromObject({
          "coin_name.$": "$.coin_name", // this works fine only if we set outputPath to $ in Map state
        }),
